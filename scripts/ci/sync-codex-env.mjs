...
import { CODEX_ENV_PLUGIN_TARGETS } from "../lib/plugin-targets.mjs";

const REPO_ROOT = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "..", "..");
const SOURCE = path.join(REPO_ROOT, "scripts/lib/codex-env.mjs");
const COPIES = CODEX_ENV_PLUGIN_TARGETS.map((plugin) =>
...
if (failures.length > 0) {
  process.stderr.write(`error: codex-env packaging copies are stale: ${failures.join(", ")}\n`);
  process.exit(1);
}
