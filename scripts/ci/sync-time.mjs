...
const SOURCE = path.join(REPO_ROOT, "scripts/lib/time.mjs");
const COPIES = [
  ...COMPANION_PLUGIN_TARGETS.map((plugin) =>
    path.join(REPO_ROOT, `plugins/${plugin}/scripts/lib/time.mjs`)
  ),
  path.join(REPO_ROOT, "plugins/api-reviewers/scripts/lib/time.mjs"),
  path.join(REPO_ROOT, "plugins/grok/scripts/lib/time.mjs"),
];
...
if (failures.length > 0) {
  process.stderr.write(`error: time packaging copies are stale: ${failures.join(", ")}\n`);
  process.exit(1);
}
