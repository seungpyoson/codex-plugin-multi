...
const SOURCE = path.join(REPO_ROOT, "scripts/lib/diff-source.mjs");
const COPIES = [
  ...COMPANION_PLUGIN_TARGETS.map((plugin) =>
    path.join(REPO_ROOT, `plugins/${plugin}/scripts/lib/diff-source.mjs`)
  ),
  path.join(REPO_ROOT, "plugins/api-reviewers/scripts/lib/diff-source.mjs"),
  path.join(REPO_ROOT, "plugins/grok/scripts/lib/diff-source.mjs"),
];
...
if (failures.length > 0) {
  process.stderr.write(`error: diff-source packaging copies are stale: ${failures.join(", ")}\n`);
  process.exit(1);
}
