...
const SOURCE = path.join(REPO_ROOT, "scripts/lib/review-prompt.mjs");
const COPIES = REVIEW_PROMPT_PLUGIN_TARGETS.map((plugin) =>
  path.join(REPO_ROOT, `plugins/${plugin}/scripts/lib/review-prompt.mjs`)
);
...
if (failures.length > 0) {
  process.stderr.write(`error: review-prompt packaging copies are stale: ${failures.join(", ")}\n`);
  process.exit(1);
}
