...
`<plugin-root>` is `plugins/gemini` or an absolute path to that plugin directory. Use skill `relay-gemini:gemini-review`. Command doc: `plugins/gemini/commands/gemini-review.md`.
...
Run `node "<plugin-root>/scripts/gemini-companion.mjs" run --mode=review --foreground --lifecycle-events markdown --cwd "<workspace>" --scope-base REF -- "<focus>"`.
