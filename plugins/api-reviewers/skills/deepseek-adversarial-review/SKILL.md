...
```bash
node plugins/api-reviewers/scripts/api-reviewer.mjs approval-request --provider deepseek --mode adversarial-review --scope branch-diff --prompt "<focus>"
node plugins/api-reviewers/scripts/api-reviewer.mjs run --provider deepseek --mode adversarial-review --scope branch-diff --approval-token "<approval_token.value>" --lifecycle-events jsonl --prompt "<focus>"
```
