...
Use skill `api-reviewers:deepseek-adversarial-review`. Command doc: `plugins/api-reviewers/commands/deepseek-adversarial-review.md`.
Scope: `branch-diff`.
`<focus>` is the user's review prompt or focus area.
Run `node plugins/api-reviewers/scripts/api-reviewer.mjs run --provider deepseek --mode adversarial-review --scope branch-diff --scope-base REF --approval-token "<approval_token.value>" --lifecycle-events markdown --prompt "<focus>"`.
## Review Contract
