...
```bash
node plugins/api-reviewers/scripts/api-reviewer.mjs approval-request --provider glm --mode review --scope branch-diff --prompt "<focus>"
node plugins/api-reviewers/scripts/api-reviewer.mjs run --provider glm --mode review --scope branch-diff --approval-token "<approval_token.value>" --lifecycle-events jsonl --prompt "<focus>"
```
