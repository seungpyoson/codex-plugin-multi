...
allowed-tools: Read, Glob, Grep, Bash(api-reviewer:*), Bash(git:*), AskUserQuestion
---
...
Run `api-reviewer approval-request --provider deepseek --mode adversarial-review --scope branch-diff --scope-base REF --prompt "<prompt text>"`.
Run `api-reviewer run --provider deepseek --mode adversarial-review --scope branch-diff --scope-base REF --approval-token "<approval_token.value>" --lifecycle-events markdown --prompt "<prompt text>"`.
## Review Contract
