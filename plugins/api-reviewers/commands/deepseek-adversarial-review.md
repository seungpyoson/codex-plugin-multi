...
```bash
node plugins/api-reviewers/scripts/api-reviewer.mjs approval-request --provider deepseek --mode adversarial-review --scope branch-diff --prompt "<prompt text>"
node plugins/api-reviewers/scripts/api-reviewer.mjs run --provider deepseek --mode adversarial-review --scope branch-diff --approval-token "<approval_token.value>" --lifecycle-events jsonl --prompt "<prompt text>"
```
