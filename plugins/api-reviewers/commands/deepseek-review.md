...
Run `node ../scripts/api-reviewer.mjs approval-request --provider deepseek --mode review --scope branch-diff --scope-base REF --prompt "<prompt text>"`.
Run `node ../scripts/api-reviewer.mjs run --provider deepseek --mode review --scope branch-diff --scope-base REF --approval-token "<approval_token.value>" --lifecycle-events markdown --prompt "<prompt text>"`.
## Review Contract
