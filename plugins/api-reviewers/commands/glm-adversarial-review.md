...
Run `api-reviewer approval-request --provider glm --mode adversarial-review --scope branch-diff --scope-base REF --prompt "<prompt text>"`.
Run `api-reviewer run --provider glm --mode adversarial-review --scope branch-diff --scope-base REF --approval-token "<approval_token.value>" --lifecycle-events markdown --prompt "<prompt text>"`.
## Review Contract
