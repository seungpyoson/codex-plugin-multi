...
`<plugin-root>` is `plugins/kimi` or an absolute path to that plugin directory. Use skill `relay-kimi:kimi-result`. Command doc: `plugins/kimi/commands/kimi-result.md`.
...
Run `node "<plugin-root>/scripts/kimi-companion.mjs" result --job "<job-id>" --cwd "<workspace>"`.
