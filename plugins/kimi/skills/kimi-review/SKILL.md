...
`<plugin-root>` is `plugins/kimi` or an absolute path to that plugin directory. Use skill `kimi:kimi-review`. Command doc: `plugins/kimi/commands/kimi-review.md`.
...
Run `node "<plugin-root>/scripts/kimi-companion.mjs" run --mode=review --foreground --lifecycle-events jsonl --cwd "<workspace>" --scope-base REF -- "<focus>"`.
