...
description: Use when asking Google Antigravity CLI to challenge a design or diff adversarially.
user-invocable: true
---
...
`<plugin-root>` is `plugins/agy` or an absolute path to that plugin directory. Use skill `relay-agy:agy-adversarial-review`. Command doc: `plugins/agy/commands/agy-adversarial-review.md`.
`<workspace>` is the repository or bundle directory to review.
`<focus>` is the user's review prompt or focus area.
Review timeout defaults to 900000 ms. Use `--timeout-ms <ms>` or `AGY_REVIEW_TIMEOUT_MS`; the effective value is persisted in `review_metadata.audit_manifest.request.timeout_ms`.

Run `node "<plugin-root>/scripts/agy-companion.mjs" run --mode=adversarial-review --foreground --lifecycle-events markdown --cwd "<workspace>" --scope-base REF -- "<focus>"`.
