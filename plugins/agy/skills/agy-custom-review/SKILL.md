---
name: agy-custom-review
description: Use when asking Google Antigravity CLI to review explicit files.
user-invocable: true
---
...
`<plugin-root>` is `plugins/agy` or an absolute path to that plugin directory. Use skill `relay-agy:agy-custom-review`. Command doc: `plugins/agy/commands/agy-custom-review.md`.
...
Run `node "<plugin-root>/scripts/agy-companion.mjs" run --mode=custom-review --foreground --lifecycle-events markdown --cwd "<workspace>" --scope-paths "<file1>,<file2>" -- "<focus>"`.
## Review Contract
