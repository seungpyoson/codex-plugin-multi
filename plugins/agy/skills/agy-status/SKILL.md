---
name: agy-status
description: Use when listing active or recent AGY-plugin jobs.
user-invocable: true
---
...
`<plugin-root>` is `plugins/agy` or an absolute path to that plugin directory. Use skill `relay-agy:agy-status`. Command doc: `plugins/agy/commands/agy-status.md`.
`<workspace>` is the workspace where the job was launched.
Run `node "<plugin-root>/scripts/agy-companion.mjs" status --cwd "<workspace>"`.
