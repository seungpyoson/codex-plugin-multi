---
name: agy-review
description: Use when asking Google Antigravity CLI to review the current diff, files, or focus area.
user-invocable: true
---
...
`<plugin-root>` is `plugins/agy` or an absolute path to that plugin directory. Use skill `relay-agy:agy-review`. Command doc: `plugins/agy/commands/agy-review.md`.
...
Run `node "<plugin-root>/scripts/agy-companion.mjs" run --mode=review --foreground --lifecycle-events markdown --cwd "<workspace>" --scope-base REF -- "<focus>"`.
