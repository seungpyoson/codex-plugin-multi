---
description: Ask DeepSeek API to propose a patch for the current branch diff.
argument-hint: "[--scope-base REF] [rescue task]"
...
## Entrypoint Contract
Use the global installed entrypoint `node "${CODEX_HOME:-$HOME/.codex}/plugins/cache/relay-for-codex/relay-deepseek/0.1.0/scripts/api-reviewer.mjs"`.
Do not run bare `api-reviewer`, do not rely on `PATH`, and do not use repository-relative paths such as `plugins/relay-deepseek/scripts/api-reviewer.mjs`.
If `${CODEX_HOME:-$HOME/.codex}/plugins/cache/relay-for-codex/relay-deepseek/0.1.0/scripts/api-reviewer.mjs` cannot be resolved, stop and report `api_reviewer_entrypoint_missing` before any source-bearing command.

Scope: `branch-diff`. Preserve raw `$ARGUMENTS` except for documented routing.
`$ARGUMENTS` is optional `--scope-base REF` followed by rescue task text.
Route `--scope-base REF` before `--prompt` and pass the remaining prompt text to `--prompt`.
Run `node "${CODEX_HOME:-$HOME/.codex}/plugins/cache/relay-for-codex/relay-deepseek/0.1.0/scripts/api-reviewer.mjs" approval-request --provider deepseek --mode rescue --scope branch-diff --scope-base REF --prompt "<prompt text>"`.
Run `node "${CODEX_HOME:-$HOME/.codex}/plugins/cache/relay-for-codex/relay-deepseek/0.1.0/scripts/api-reviewer.mjs" run --provider deepseek --mode rescue --scope branch-diff --scope-base REF --approval-token "<approval_token.value>" --lifecycle-events markdown --prompt "<prompt text>"`.
...
After a completed DeepSeek rescue proposal, run `node "${CODEX_HOME:-$HOME/.codex}/plugins/cache/relay-for-codex/relay-deepseek/0.1.0/scripts/api-reviewer.mjs" apply-request --job-id "<rescue_job_id>"`.
Render the apply-request output and request explicit approval with its `recommended_tool_justification`.
Only after approval, run `node "${CODEX_HOME:-$HOME/.codex}/plugins/cache/relay-for-codex/relay-deepseek/0.1.0/scripts/api-reviewer.mjs" apply --job-id "<rescue_job_id>" --approval-token "<approval_token.value>"`.
...
This command backs `../skills/deepseek-rescue/SKILL.md`.
