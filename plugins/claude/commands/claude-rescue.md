---
description: Delegate investigation or a fix to Claude Code. Supports foreground or background runs.
...
- `node "<plugin-root>/scripts/claude-companion.mjs" run --mode=rescue --auth-mode subscription --background --lifecycle-events markdown -- "$ARGUMENTS"`

For quick attached work, run in the foreground:

- `node "<plugin-root>/scripts/claude-companion.mjs" run --mode=rescue --auth-mode subscription --foreground --lifecycle-events markdown -- "$ARGUMENTS"`
...
This command backs `plugins/claude/skills/claude-rescue/SKILL.md`.
