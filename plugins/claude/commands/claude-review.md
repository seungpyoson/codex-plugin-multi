...
2. Run:
   ```
   node "<plugin-root>/scripts/claude-companion.mjs" run --mode=review --foreground --auth-mode subscription --lifecycle-events jsonl -- "<focus text>"
   ```
   ...
   `run --mode=custom-review --auth-mode subscription --scope-paths <g1,g2,...>` and
   refer to files by relative paths inside the selected scope.
