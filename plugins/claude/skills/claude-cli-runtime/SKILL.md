...
- **Review auth**: command skills should pass `--auth-mode subscription` for Claude review, adversarial-review, custom-review, and setup checks. Subscription mode ignores Claude provider API-key env vars and proves the OAuth/subscription inference path. Use `--auth-mode api_key` only when the user explicitly requests API-key auth; `--auth-mode auto` is a compatibility mode that may select API-key auth when a provider key is present.
