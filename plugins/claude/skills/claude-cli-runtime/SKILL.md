...
- **Git-derived scopes**: `staged`, `head`, and `branch-diff` are object-pure; checkout filters, replace refs, and grafts are ignored. `working-tree` reflects live filesystem content for tracked + untracked-non-ignored files only — gitignored files (e.g. `.env`) are excluded by default for privacy; use `custom` with explicit globs when ignored files must be included.
