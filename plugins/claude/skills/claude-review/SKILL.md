...
`<plugin-root>` is `plugins/claude` or an absolute path to that plugin directory. Use skill `relay-claude:claude-review`. Command doc: `plugins/claude/commands/claude-review.md`.
...
Run `node "<plugin-root>/scripts/claude-companion.mjs" run --mode=review --auth-mode subscription --foreground --lifecycle-events markdown --cwd "<workspace>" --scope-base REF -- "<focus>"`.
