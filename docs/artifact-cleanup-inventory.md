...
Applies to DeepSeek and GLM `review`, `adversarial-review`, `custom-review`, and API-backed `rescue` direct API runs.
...
| `jobs/<jobId>/rescue-patch.json` | API reviewer data root | Private unredacted rescue patch artifact keyed by completed rescue proposal job | Source-free `apply-request` and `apply` validation after the public JobRecord is redacted | With the owning rescue proposal job directory | `api-reviewer.mjs` |
...
| `jobs/<jobId>/rescue-patch.json.*.tmp` | API reviewer job dir | Partial atomic rescue patch write | Nothing after interrupted write | Removed on write failure; containing job dir removed on prune | `api-reviewer.mjs` |
| `approval-tokens/rescue-apply/<sha256>.json` | API reviewer data root | Source-free local apply approval token metadata, bound to rescue job, workspace root, HEAD, and patch hash | One approved local `apply` command | Consumed token stays marked used until normal data-root cleanup | `api-reviewer.mjs` |
...
DeepSeek/GLM branch-diff and custom-review material is read from the workspace into memory and sent in the HTTP request body. API-backed rescue uses the same source-send approval for proposal generation, then stores the provider's unredacted unified diff in a private per-job rescue patch artifact so the public JobRecord can stay source-redacted while local apply remains possible. `apply-request` and `apply` are source-free local commands; they do not send selected source to an external provider. The API reviewer path does not persist prompt sidecars, copied review bundles, branch-diff files, stdout/stderr logs, PID records, cancel markers, or subprocess state. Git scope discovery uses synchronous child processes that complete before the JobRecord is built; provider execution uses `fetch` in the current process. There is no live provider-owned subprocess to terminate on prune.
...
| Artifact | Root | Contents | Needed for | Safe deletion point | Owner |
| --- | --- | --- | --- | --- | --- |
| `state.json` | `GROK_PLUGIN_DATA` or `$TMPDIR/relay/grok/<workspace-slug>-<hash>` | Recent Grok job summaries, newest first | `grok-web-reviewer.mjs list` and local diagnostics | Never as per-job cleanup | `grok-web-reviewer.mjs` |
