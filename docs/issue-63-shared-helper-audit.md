...
| Companion-local utilities (`printJson`, prompt sidecars, run-kind helpers, status/result/cancel glue) | Exact duplicate names appear in companion entry points, but call sites are embedded in provider-specific command flows. | Claude, Gemini, and Kimi companion entry points. | Leave local. | Extracting these would move command orchestration into a generic helper with many provider-specific parameters. That increases coupling without proving behavior safer. |
