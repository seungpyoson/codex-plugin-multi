...
Git-derived scopes (`staged`, `branch-diff`, `head`) are **object-pure snapshots**. They do not run `git checkout`, `git checkout-index`, checkout filters, clean/smudge/process filters, LFS smudge, textconv, hooks, `.gitattributes` transformations, replace refs, or config-defined shell commands during scope population. They represent canonical git object bytes, not post-filter working-tree bytes. Use `working-tree` or `custom` when the target must see the user's live filesystem representation.
