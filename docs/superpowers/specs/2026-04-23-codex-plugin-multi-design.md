...
Git-derived scopes (`staged`, `branch-diff`, `head`) are **object-pure snapshots**. They do not run `git checkout`, `git checkout-index`, checkout filters, clean/smudge/process filters, LFS smudge, textconv, hooks, `.gitattributes` transformations, replace refs, grafts, or config-defined shell commands during scope population. They represent canonical git object bytes, not post-filter working-tree bytes. The companion assumes Git 2.x or newer for these object-pure environment controls. Supported reviewer execution platforms are macOS and Linux; Windows support is out of scope for this contract. Scope collection defaults to `/usr/bin/git` and never resolves `git` from the caller's ambient `PATH`. Environments that install Git elsewhere must set `CODEX_PLUGIN_MULTI_GIT_BINARY` to an absolute executable path outside the current workspace and outside `node_modules/.bin`. Use `working-tree` or `custom` when the target must see the user's live filesystem representation.
