...
- **Tracks spec:** `docs/superpowers/specs/2026-04-23-codex-plugin-multi-design.md` (v4)
- **Branch:** `docs/1-design-spec` (spec + plan merge together)
...
```
M0 scaffold      → T0.1 → T0.2 → T0.3 → T0.4 → T0.5 → T0.6
                                                           ⚠️ gate-0
M1 shared-lib    → T1.1 → T1.2 → T1.3 → T1.4 → T1.5 → T1.6
                                                           ⚠️ gate-1
M2 claude fg     → T2.1 → T2.2 → T2.3 → T2.4 → T2.5
M3 claude cmds   → T3.1 → T3.2 → T3.3 → T3.4 → T3.5 → T3.6
                                                           ⚠️ gate-2
M4 claude bg     → T4.1 → T4.2 → T4.3 → T4.4
M5 claude iso    → T5.1 → T5.2 → T5.3 → T5.4
M6 claude skill  → T6.1 → T6.2 → T6.3
                                                           ⚠️ gate-3
M7 gemini port   → T7.1 → T7.2 → T7.3 → T7.4 → T7.5
M8 gemini bg     → T8.1 → T8.2 → T8.3
                                                           ⚠️ gate-4
M9 tests + CI    → T9.1 → T9.2 → T9.3 → T9.4 → T9.5
M10 release      → T10.1 → T10.2 → T10.3 → T10.4
...
⚠️ **gate-3:** Adversarial review. Focus: dispose-path edge cases (cleanup on crash), skill content correctness, prompting-skill completeness.

---

## M7 — Gemini port (policy-first)

**Milestone goal:** symmetric Gemini plugin with TOML policy as the real enforcement.

### T7.1 — `gemini-mock.mjs` fixture CLI
...
- **Acceptance:** Unit test green. File is loaded by `gemini-mock.mjs` in T7.1 without errors.
- **Spec ref:** §7.3, verified §4.5.
- **Depends on:** T7.1

### T7.3 — `lib/gemini.mjs` + `gemini-companion.mjs` entry
...
### T7.5 — Gemini smoke tests (foreground)
...
### T8.1 — Gemini `run --background` + detached wrapper
...
### T8.2 — `continue --job <id>` with `--resume latest`/UUID
...
### T9.4 — manifest + schema lint matrix
...
### T9.5 — README + install instructions
...
### T10.4 — install verification on fresh machine
...
- **Branching:** spec + plan co-live on `docs/1-design-spec`. Implementation tasks may spin per-milestone branches (`feat/m0-skeleton`, `feat/m1-libs`, …) that merge into `docs/1-design-spec`, then final merge to `main` at T10.3.
