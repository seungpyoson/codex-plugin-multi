...
async function optionsWithPromptFile(options) {
  ...
  if (hasPromptText(options.prompt)) {
    throw new Error("bad_args: pass prompt either with --prompt-file or --prompt, not both");
  }
  let promptText;
  try {
    promptText = (await readFile(promptFile, "utf8")).trim();
  } catch (e) {
    throw new Error(`bad_args: could not read --prompt-file: ${e.message}`);
  }
  if (!hasPromptText(promptText)) {
    throw new Error("bad_args: --prompt-file must contain a non-empty prompt");
  }
