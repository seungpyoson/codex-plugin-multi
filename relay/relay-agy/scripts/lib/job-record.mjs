...
export function externalReviewForInvocation(invocation, execution = null) {
  ...
  return buildExternalReview({
    invocation,
    sessionId: execution?.agySessionId ?? null,
    status,
    errorCode: error_code,
    sourceContentTransmission,
 ...
 *   step_limit_exceeded — AGY emitted its non-JSON max-step exhaustion sentinel.
 *   usage_limited  — AGY emitted a non-JSON quota/usage-limit failure.
 ...
 *   review_not_completed — AGY stdout lacked a substantive review verdict.
 *   agy_error      — exitCode !== 0 with parseable JSON from AGY.
 *                     Also covers exitCode === 0 but parsed.ok === false with
 *                     is_error semantics.
 *   agy_error      — catch-all target failure; should be rare when no
...
function classifyProviderParsedFailure({ execution, parsed }) {
  ...
  if (parsed?.reason === "review_not_completed") {
    return {
      status: "failed",
...
function buildErrorDiagnostic(invocation, status, error_code, error_message) {
  ...
  const cliName = targetCliName(target);
  if (status === "failed" && error_code === "usage_limited") {
    return {
      error_summary: `${cliName} reported a subscription usage limit before returning a review result.`,
      error_cause:
        `${cliName} emitted a plain-text quota or billing-cycle failure instead of ` +
  ...
  if (status === "failed" && error_code === "not_authed") {
    return {
      ...
      suggested_action:
        `Run \`${target.binaryName} login\` in a normal terminal, rerun /agy-setup, then retry the review.`,
      disclosure_note: null,
    };
  }
  ...
  if (status === "failed" && error_code === "review_not_completed") {
    ...
    return {
      error_summary: `${cliName} review did not complete as a usable external review.`,
      error_cause:
  ...
  if (status !== "failed" || error_code !== "scope_failed" || !error_message) {
    const sharedDiagnostic = status === "failed"
      ? buildExternalModelFailureDiagnostic(error_code, cliName)
      : null;
...
function targetDisplayName(target) {
  ...
  if (target === "kimi") return { displayName: "Kimi", binaryName: "kimi" };
  return { displayName: "Google Antigravity CLI", binaryName: "agy" };
}

function targetCliName(target) {
  return target.displayName.endsWith("CLI")
    ? target.displayName
    : `${target.displayName} Code CLI`;
}
