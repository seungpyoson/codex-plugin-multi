...
Run `node "${CLAUDE_PLUGIN_ROOT}/scripts/relay-run.mjs" api-reviewer.mjs approval-request --provider deepseek --mode rescue --scope branch-diff --scope-base REF --prompt-file "$RELAY_PROMPT_FILE"`.
Run `node "${CLAUDE_PLUGIN_ROOT}/scripts/relay-run.mjs" api-reviewer.mjs run --provider deepseek --mode rescue --scope branch-diff --scope-base REF --approval-token "<approval_token.value>" --lifecycle-events markdown --prompt-file "$RELAY_PROMPT_FILE"`.
...
After a completed DeepSeek rescue proposal, run `node "${CLAUDE_PLUGIN_ROOT}/scripts/relay-run.mjs" api-reviewer.mjs apply-request --job-id "<rescue_job_id>"`.
Render the apply-request output and request explicit approval with its `recommended_tool_justification`.
Only after approval, run `node "${CLAUDE_PLUGIN_ROOT}/scripts/relay-run.mjs" api-reviewer.mjs apply --job-id "<rescue_job_id>" --approval-token "<approval_token.value>"`.
