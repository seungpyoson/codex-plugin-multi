...
Run `node "${CLAUDE_PLUGIN_ROOT}/scripts/relay-run.mjs" api-reviewer.mjs approval-request --provider deepseek --mode adversarial-review --scope branch-diff --scope-base REF --prompt-file "$RELAY_PROMPT_FILE"`.
Run `node "${CLAUDE_PLUGIN_ROOT}/scripts/relay-run.mjs" api-reviewer.mjs run --provider deepseek --mode adversarial-review --scope branch-diff --scope-base REF --approval-token "<approval_token.value>" --lifecycle-events markdown --prompt-file "$RELAY_PROMPT_FILE"`.
## Review Contract
